...
import '../api/api_outstanding_efficiency.dart';
...
class EfficiantPage extends StatefulWidget {
  const EfficiantPage({super.key});

  @override
  _EfficiantPageState createState() => _EfficiantPageState();
}

class _EfficiantPageState extends State<EfficiantPage> with SingleTickerProviderStateMixin{
 late TabController _tabController;
  ...
  String title = 'كفاءات متميزة';
  String subtitle = 'استكشفي شخصيات نساء مؤثرات';
  String womenTab = 'كفاءات';
  String childTab = 'اخرى';
  ...
  void _updateTexts(String selectedLang) {
    setState(() {
      if (selectedLang == 'ar') {
        title = 'كفاءات متميزة';
        subtitle = 'استكشفي شخصيات نساء مؤثرات';
        womenTab = 'كفاءات';
        childTab = 'اخرى';
        searchHint = 'ابحث عن مقال';
        logoutText = 'تسجيل خروج';
      } else {
        title = 'Outstanding Competencies';
        subtitle = 'Explore the personalities of influential women';
        womenTab = 'Competencies';
        childTab = 'Other';
  ...
  Future<void> _fetchother() async {
    ...
    try {
      final api = ApiNews();
      final events =
          await api.other(sectionId: userType.toString(), lang: lang);

      setState(() {
        childHealthImages =
            events.map((event) => event['file_path'] as String).toList();
        childHealthTitles =
            events.map((event) => event['title'] as String).toList();
        childHealthDescriptions =
            events.map((event) => event['content_text'] as String).toList();
        childHealthDates =
            events.map((event) => event['created_at'] as String).toList();
        _filterother();
        _isLoading = false;
      });
    } catch (e) {
        ...
        return GestureDetector(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => efficiancydetailsPage(
                  title: titles[index],
                  image: images[index],
