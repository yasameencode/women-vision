...
Future<void> _submitComplaint() async {
  ...
  try {
    ...
    if (response['success_complaint_count'] == true) {
      print('تم إرسال الاستشارة بنجاح'); // طباعة في حالة النجاح
      ScaffoldMessenger.of(context).showSnackBar(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(
                'فشل تقديم الاستشارة: ${response['error_complaint_count'] ?? 'غير محدد'}')),
      );
    }
  } catch (e) {
    print('حدث خطأ أثناء تقديم الاستشارة: $e'); // طباعة عند حدوث استثناء
    ScaffoldMessenger.of(context).showSnackBar(
    ...
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text('هل أنت متأكد من تقديم الاستشارة؟'),
          actions: <Widget>[
            Row(
              mainAxisAlignment:
