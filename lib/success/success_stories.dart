...
Future<void> _submitComplaint() async {
  ...
  try {
    ...
    if (response['success_complaint_count'] == true) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('تم تقديم القصة بنجاح')),
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(
                'فشل تقديم القصة: ${response['error_complaint_count'] ?? 'غير محدد'}')),
      );
    }
  } catch (e) {
    print('حدث خطأ أثناء تقديم الشكوى: $e'); // طباعة عند حدوث استثناء
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('حدث خطأ أثناء تقديم القصة: ${e.toString()}')),
    );
  }
}
    ...
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text('هل أنت متأكد من تقديم القصة؟'),
          actions: <Widget>[
            Row(
              mainAxisAlignment:
