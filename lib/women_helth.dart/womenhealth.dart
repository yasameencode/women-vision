...
import '../api/api_women_health.dart';
...
class WhealthPage extends StatefulWidget {
  ...
  @override
  _WhealthPageState createState() => _WhealthPageState();
}

class _WhealthPageState extends State<WhealthPage>with SingleTickerProviderStateMixin {
 late TabController _tabController; 
  ...
  String title = 'صحة المرأة والطفل';
  String subtitle = 'استكشفي مقالات عن صحتك أو صحة طفلك';
  String womenTab = 'صحة المرأة';
  String childTab = 'صحة الطفل';
  ...
  void _updateTexts(String selectedLang) {
    setState(() {
      if (selectedLang == 'ar') {
        title = 'صحة المرأة والطفل';
        subtitle = 'استكشفي مقالات عن صحتك أو صحة طفلك';
        womenTab = 'صحة المرأة';
        childTab = 'صحة الطفل';
        ...
        subtitle = 'Explore articles about your health or your child’s health';
        womenTab = 'Women Health';
        childTab = 'Child Health';
  ...
  Future<void> _fetchWomenHealth() async {
    ...
    try {
      final api = ApiNews();
      final articles = await api.womenhealth(sectionId: userType.toString(), lang: lang);
  ...
  Future<void> _fetchChildHealth() async {
    if (userType == null) return;
    ...
    try {
      final api = ApiNews();
      final events = await api.chiledhealth(sectionId: userType.toString(), lang: lang);
        ...
        return GestureDetector(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => WhealthdetailsPage(
                  title: titles[index],
                  image: images[index],
