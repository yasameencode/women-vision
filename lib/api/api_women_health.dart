...
Future<List<dynamic>> womenhealth({required String sectionId, required String lang}) async {
  final response = await http.post(
    ...
    body: jsonEncode({
      ...
      "t_id": 11       // t_id=11 خاص بصحة المرأة
    }),
  );
  ...
  if (response.statusCode == 200) {
    ...
    throw Exception('Failed to load women health data');
  }
}
...
Future<List<dynamic>> chiledhealth({required String sectionId, required String lang}) async {
  final response = await http.post(
    Uri.parse(_baseUrl),  // استخدم POST مع URL الأساسي
    headers: {"Content-Type": "application/json"},  // تحديد نوع المحتوى كـ JSON
    body: jsonEncode({
      ...
      "lang": lang,
      "role": "user",  // الدور هنا هو "user"
      "t_id": 12       // t_id=12 خاص بصحة الطفل
    }),
  );
  ...
  if (response.statusCode == 200) {
    ...
    throw Exception('Failed to load child health data');
  }
}
